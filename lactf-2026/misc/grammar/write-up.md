...
From this production rule and the provided parse tree, we can further reason about the production rule of the flag, which can be expanded as to `start, word, underscore, word, underscore, word, end`. Since we also know that `start`, `underscore`, and `end` are production rules for terminal characters, we only need to figure out how each `word` of the flag was structured. From these information, we can partially fill in the parse tree.

![partial](https://raw.githubusercontent.com/ssung099/CTF-Writeups/main/lactf-2026/misc/grammar/partial.png)
...
We can repeat a similar step for the green and purple fragments. Looking at the leftmost parse subtree for purple fragment, we can see that the second terminal character is a result of five nonterminals. Given the remaining candidates `vc` and `vd`, the second terminal character must be a product of `c` or `d`. Once again, since `d` can only produce at most four nonterminals, it must be that the second terminal character for purple was produced by the rule for `c` and therefore, purple represents the fragment production for `vc`. Since only `vd` remains, we also now know that green was the representation for `vd`.
